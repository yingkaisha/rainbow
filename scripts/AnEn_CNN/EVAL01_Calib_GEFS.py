...
for THRES in np.array([5, 30.0]):
    ...
    prob_pred = np.sum(PRED>thres, axis=2)/EN
    ...
    du.save_hdf5(tuple_save, label_save, save_dir, 'GEFS_Calib-{}mm_{}_{}.hdf'.format(int(THRES), year0, year1))
...
for p in np.array([0.9,]):
    ...
    prob_pred = np.sum(PRED_binary, axis=2)/EN
    ...
    tuple_save = (brier_all, pos_frac_all, pred_value_all, use_all, o_bar_all)
    label_save = ['brier', 'pos_frac', 'pred_value', 'use', 'o_bar']
    du.save_hdf5(tuple_save, label_save, save_dir, 'GEFS_Calib-{}th_{}_{}.hdf'.format(int(p*100), year0, year1))
